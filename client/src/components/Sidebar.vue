  ...
  methods: {
    ...
    async loadUserData() {
      try {
        ...
        if (accountData) {
          ...
          this.userAvatar = accountData.avatar || null;
        }
    ...
    getAvatarUrl() {
      ...
      if (this.avatarError || !this.userAvatar) {
        console.log('Using default avatar - avatarError:', this.avatarError, 'userAvatar:', this.userAvatar);
        return this.defaultAvatar;
      }
    
      // If userAvatar is a full URL, use it directly
      if (this.userAvatar.startsWith('http')) {
        return this.userAvatar;
      }
    
      // Handle avatar path for uploaded files
      let avatarPath = this.userAvatar.toString().trim();
    
      // Additional validation
      if (!avatarPath || avatarPath.length === 0) {
        console.log('Empty avatar path, using default');
        return this.defaultAvatar;
      }
    
      // Clean up any path issues
      avatarPath = avatarPath.replace(/^\/+/, ''); // Remove leading slashes
    
      // Use relative URL to leverage the proxy
      const avatarUrl = `/uploads/avatars/${avatarPath}`;
      console.log('Using custom avatar URL:', avatarUrl);
      return avatarUrl;
    },
