  ...
  methods: {
    ...
    async processAudio() {
      console.log('processAudio called with:', {
        selectedAudioFile: this.selectedAudioFile,
        audioBlob: this.audioBlob,
        audioBlobSize: this.audioBlob?.size || 0
      });
      ...
      // Additional safety check for audioBlob
      if (!this.selectedAudioFile && (!this.audioBlob || this.audioBlob.size === 0)) {
        this.showError('No valid audio data available for processing');
        return;
      }
      ...
      try {
        ...
        // Convert blob to file if using recorded audio
        let audioFile = this.selectedAudioFile;
        if (!audioFile && this.audioBlob) {
          console.log('Converting audioBlob to File:', this.audioBlob.size, 'bytes');
          audioFile = new File([this.audioBlob], 'recording.wav', { type: 'audio/wav' });
          console.log('Created audioFile:', audioFile.name, audioFile.size, audioFile.type);
        }

        // Final validation
        if (!audioFile || audioFile.size === 0) {
          throw new Error('No valid audio file available for processing');
        }
